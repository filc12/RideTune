...
const en: LegalDict = {
  ...
  privacySections: [
    ...
    ["Purchases", "RideTune Premium Lifetime is a one-time purchase processed entirely by the store you installed the app from — Google Play Billing on Android, the App Store on iOS. We use RevenueCat to validate purchases and restore them on reinstall. We receive an anonymous purchase record — never your payment details, which stay with Google or Apple."],
    ...
    ["Community setups (shared by you)", "If you choose to share a setup to the community, that setup — your motorcycle, load, sag and clicker values, an optional country and an optional short note — is published on ridetune.app as 'Anonymous rider'. It is tied only to a random device identifier generated on your phone, never to your name, email or any personal data. Sharing is always your choice and is confirmed each time. To have a shared setup removed, email support@ridetune.app with a link to it and we will delete it from our servers. Submissions may be checked automatically and lightly moderated to keep the library useful. Community data is stored with Supabase (our database provider). By sharing, you agree to the submission terms."],
    ["Data deletion", "Because your data lives on your device, deleting the app deletes your on-device data. To have a setup you shared to the community removed, email support@ridetune.app with a link to it and we will delete it from our servers. Anonymous analytics and crash data are retained by our providers for a limited period and cannot be linked back to you."],
  ...
  termsSections: [
    ...
    ["Removing your setups", "To remove a shared setup, email support@ridetune.app with a link to it and we will delete it from our servers. Because setups are anonymous and carry no personal data, we cannot tell which are yours — always include the link."],
...
const pt: LegalDict = {
  ...
  privacySections: [
    ...
    ["Compras", "O RideTune Premium Vitalício é uma compra única processada inteiramente pela loja de onde instalaste a app — Faturação da Google Play no Android, App Store no iOS. Usamos a RevenueCat para validar compras e restaurá-las ao reinstalar. Recebemos um registo de compra anónimo — nunca os teus dados de pagamento, que ficam com a Google ou com a Apple."],
    ...
    ["Setups da comunidade (partilhados por ti)", "Se optares por partilhar um setup para a comunidade, esse setup — a tua moto, carga, sag e valores de cliques, um país opcional e uma nota curta opcional — é publicado em ridetune.app como 'Condutor anónimo'. Está ligado apenas a um identificador de dispositivo aleatório gerado no teu telemóvel, nunca ao teu nome, email ou quaisquer dados pessoais. A partilha é sempre tua escolha e é confirmada de cada vez. Para remover um setup partilhado, envia um email para support@ridetune.app com a ligação para ele e apagamo-lo dos nossos servidores. As submissões podem ser verificadas automaticamente e ligeiramente moderadas para manter a biblioteca útil. Os dados da comunidade são guardados na Supabase (o nosso fornecedor de base de dados). Ao partilhar, aceitas os termos de submissão."],
    ["Eliminação de dados", "Como os teus dados vivem no teu dispositivo, apagar a app apaga os dados no dispositivo. Para remover um setup que partilhaste para a comunidade, envia um email para support@ridetune.app com a ligação para ele e apagamo-lo dos nossos servidores. Os dados anónimos de análise e de falhas são retidos pelos nossos fornecedores por um período limitado e não podem ser associados a ti."],
  ...
  termsSections: [
    ...
    ["Remover os teus setups", "Para remover um setup partilhado, envia um email para support@ridetune.app com a ligação para ele e apagamo-lo dos nossos servidores. Como os setups são anónimos e não têm dados pessoais, não conseguimos saber quais são os teus — inclui sempre a ligação."],
...
const es: LegalDict = {
  ...
  privacySections: [
    ...
    ["Compras", "RideTune Premium de por vida es una compra única procesada íntegramente por la tienda desde la que instalaste la app — Facturación de Google Play en Android, App Store en iOS. Usamos RevenueCat para validar las compras y restaurarlas al reinstalar. Recibimos un registro de compra anónimo — nunca tus datos de pago, que se quedan con Google o con Apple."],
    ...
    ["Ajustes de la comunidad (compartidos por ti)", "Si eliges compartir un ajuste con la comunidad, ese ajuste — tu moto, carga, sag y valores de clics, un país opcional y una nota corta opcional — se publica en ridetune.app como 'Piloto anónimo'. Está vinculado solo a un identificador de dispositivo aleatorio generado en tu teléfono, nunca a tu nombre, correo ni datos personales. Compartir es siempre tu elección y se confirma cada vez. Para eliminar un ajuste compartido, escribe a support@ridetune.app con el enlace y lo borraremos de nuestros servidores. Las publicaciones pueden verificarse automáticamente y moderarse ligeramente para mantener útil la biblioteca. Los datos de la comunidad se almacenan con Supabase (nuestro proveedor de base de datos). Al compartir, aceptas los términos de publicación."],
    ["Eliminación de datos", "Como tus datos residen en tu dispositivo, eliminar la app elimina los datos del dispositivo. Para eliminar un ajuste que compartiste con la comunidad, escribe a support@ridetune.app con el enlace y lo borraremos de nuestros servidores. Los datos anónimos de analítica y errores se conservan con nuestros proveedores durante un periodo limitado y no pueden vincularse a ti."],
  ...
  termsSections: [
    ...
    ["Eliminar tus ajustes", "Para eliminar un ajuste compartido, escribe a support@ridetune.app con el enlace y lo borraremos de nuestros servidores. Como los ajustes son anónimos y no llevan datos personales, no podemos saber cuáles son los tuyos: incluye siempre el enlace."],
...
const fr: LegalDict = {
  ...
  privacySections: [
    ...
    ["Achats", "RideTune Premium à vie est un achat unique traité entièrement par la boutique depuis laquelle vous avez installé l'application — Facturation Google Play sur Android, App Store sur iOS. Nous utilisons RevenueCat pour valider les achats et les restaurer lors d'une réinstallation. Nous recevons un enregistrement d'achat anonyme — jamais vos données de paiement, qui restent chez Google ou Apple."],
  ...
  termsSections: [
    ...
    ["Supprimer vos réglages", "Pour faire retirer un réglage partagé, écrivez à support@ridetune.app en joignant son lien et nous le supprimerons de nos serveurs. Comme les réglages sont anonymes et ne contiennent aucune donnée personnelle, nous ne pouvons pas savoir lesquels sont les vôtres — joignez toujours le lien."],
...
const de: LegalDict = {
  ...
  privacySections: [
    ...
    ["Käufe", "RideTune Premium Lifetime ist ein einmaliger Kauf, der vollständig über den Store abgewickelt wird, aus dem du die App installiert hast — Google Play Billing unter Android, App Store unter iOS. Wir nutzen RevenueCat, um Käufe zu validieren und bei einer Neuinstallation wiederherzustellen. Wir erhalten einen anonymen Kaufnachweis — niemals deine Zahlungsdaten, die bei Google oder Apple bleiben."],
    ...
    ["Community-Setups (von dir geteilt)", "Wenn du ein Setup mit der Community teilst, wird dieses Setup — dein Motorrad, die Last, Sag- und Klicker-Werte, ein optionales Land und eine optionale kurze Notiz — auf ridetune.app als 'Anonymer Fahrer' veröffentlicht. Es ist nur mit einer zufälligen, auf deinem Telefon erzeugten Gerätekennung verknüpft, niemals mit deinem Namen, deiner E-Mail oder persönlichen Daten. Das Teilen ist immer deine Entscheidung und wird jedes Mal bestätigt. Um ein geteiltes Setup entfernen zu lassen, schreibe an support@ridetune.app und füge den Link dazu bei; wir löschen es dann von unseren Servern. Einreichungen können automatisch geprüft und leicht moderiert werden, damit die Bibliothek nützlich bleibt. Community-Daten werden bei Supabase (unserem Datenbankanbieter) gespeichert. Mit dem Teilen akzeptierst du die Einreichungsbedingungen."],
    ["Datenlöschung", "Da deine Daten auf deinem Gerät liegen, löscht das Deinstallieren der App die Daten auf dem Gerät. Um ein Setup entfernen zu lassen, das du mit der Community geteilt hast, schreibe an support@ridetune.app und füge den Link dazu bei; wir löschen es dann von unseren Servern. Anonyme Analyse- und Absturzdaten werden von unseren Anbietern für einen begrenzten Zeitraum aufbewahrt und können nicht mit dir verknüpft werden."],
  ...
  termsSections: [
    ...
    ["Deine Setups entfernen", "Um ein geteiltes Setup entfernen zu lassen, schreibe an support@ridetune.app und füge den Link dazu bei; wir löschen es dann von unseren Servern. Da Setups anonym sind und keine personenbezogenen Daten enthalten, können wir nicht erkennen, welche deine sind — füge daher immer den Link bei."],
...
const it: LegalDict = {
  ...
  privacySections: [
    ...
    ["Acquisti", "RideTune Premium a vita è un acquisto unico gestito interamente dallo store da cui hai installato l'app — Google Play Billing su Android, App Store su iOS. Usiamo RevenueCat per convalidare gli acquisti e ripristinarli in caso di reinstallazione. Riceviamo una registrazione d'acquisto anonima — mai i tuoi dati di pagamento, che restano a Google o ad Apple."],
    ...
    ["Setup della community (condivisi da te)", "Se scegli di condividere un setup con la community, quel setup — la tua moto, il carico, il sag e i valori di clic, un paese facoltativo e una breve nota facoltativa — viene pubblicato su ridetune.app come 'Pilota anonimo'. È legato solo a un identificatore di dispositivo casuale generato sul tuo telefono, mai al tuo nome, email o dati personali. La condivisione è sempre una tua scelta ed è confermata ogni volta. Per far rimuovere un setup condiviso, scrivi a support@ridetune.app allegando il link e lo elimineremo dai nostri server. Gli invii possono essere verificati automaticamente e moderati leggermente per mantenere utile la libreria. I dati della community sono conservati su Supabase (il nostro fornitore di database). Condividendo, accetti i termini di invio."],
    ["Eliminazione dei dati", "Poiché i tuoi dati risiedono sul tuo dispositivo, eliminare l'app elimina i dati sul dispositivo. Per far rimuovere un setup che hai condiviso con la community, scrivi a support@ridetune.app allegando il link e lo elimineremo dai nostri server. I dati anonimi di analisi e crash sono conservati dai nostri fornitori per un periodo limitato e non possono essere ricollegati a te."],
  ...
  termsSections: [
    ...
    ["Rimuovere i tuoi setup", "Per far rimuovere un setup condiviso, scrivi a support@ridetune.app allegando il link e lo elimineremo dai nostri server. Poiché i setup sono anonimi e non contengono dati personali, non possiamo sapere quali siano i tuoi: allega sempre il link."],
