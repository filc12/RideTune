...
import { saveSetup } from "@/src/utils/setups";
import { isPremium } from "@/src/services/premium";
import { canUseLoadMode } from "@/src/services/premium";
...
export default function HomeScreen() {
  ...
  const onSwitchSave = useCallback(async () => {
    ...
    try {
      const curSetup = calcSetupById(bike.id, load);
      await saveSetup({
        name: `${bikeLabel(bike.id)} — ${new Date().toLocaleDateString()}`,
        bikeId: bike.id,
        bikeLabel: bikeLabel(bike.id),
        load,
        setup: curSetup,
        confidence: curSetup.confidence,
      });
    } catch {}
    setSwitchModal({ visible: false, target: null });
  ...
  const onSwitchDiscard = useCallback(async () => {
    const target = switchModal.target;
    setSwitchModal({ visible: false, target: null });
    if (target) await doSwitchBike(target);
  }, [switchModal.target, doSwitchBike]);
  ...
  return (
    <View style={styles.root} testID="home-screen">
              ...
              {!bike ? (
                ...
                <>
                  ...
                  <View style={styles.sagRow}>
                    ...
                    {(() => {
                      const sagOk = setup.sag >= 25 && setup.sag <= 75;
      ...
      <SwitchBikeModal
        ...
        fromLabel={bike ? `${bike.brand} ${bike.model}` : undefined}
        toLabel={switchModal.target ? `${switchModal.target.brand} ${switchModal.target.model}` : undefined}
        onSave={onSwitchSave}
...
function rtNorm(s: string): string {
...
function BikePicker({ open, onClose, onPick, selectedId, t }: { open: boolean; onClose: () => void; onPick: (b: Bike) => void; selectedId?: string; t: (k: never) => string }) {
  ...
  const reset = () => { setStep("cat"); setSelCat(null); setSelBrand(null); setQuery(""); };
      ...
      <View style={styles.sheet} testID="bike-picker">
        ...
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder={t("picker.search_ph" as never)}
          ...
          />
          {query.length > 0 && (
            <HapticButton onPress={() => setQuery("")} style={{ padding: 2 }} haptic="none">
              <Ionicons name="close-circle" size={18} color={C.textMute} />
            </HapticButton>
          )}
        </View>
        <Text style={styles.sheetSub}>
          {q.length >= 1 ? `${searchResults.length} ${searchResults.length === 1 ? "resultado" : "resultados"}` :
        ...
        <ScrollView style={{ maxHeight: kbHeight > 0 ? 300 : 540 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          {q.length >= 1 && searchResults.map((b) => {
            ...
            return (
              <HapticButton key={b.id} activeOpacity={0.85} onPress={() => handlePick(b)} haptic="none"
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.bikeBrand}>{b.brand} {b.model}</Text>
          ...
          {q.length === 0 && step === "model" && modelsInBrandCat.map((b) => {
            const active = b.id === selectedId;
            return (
              <HapticButton key={b.id} activeOpacity={0.85} onPress={() => handlePick(b)} haptic="none"
                style={[styles.bikeRow, active && styles.bikeRowActive]} testID={`bike-${b.id}`}>
